...
This file defines the audit process for the Security Auditor. It provides
phase-level guidance without duplicating agent responsibilities, domain lists,
or finding structures.

## Purpose

Guide the Security Auditor through a repeatable, evidence-based process for
evaluating security posture within repository scope.
...
Confirm which security domains apply based on `applicability.md`. Mark
...
Assess maturity per security domain using the canonical maturity model levels:
...
Generate `security.md` inside the audit workspace following the structure
defined in the Security Auditor agent file.

## Boundaries

This methodology does not:

- define security domains (see Security Auditor agent);
